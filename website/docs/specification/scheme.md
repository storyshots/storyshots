...
Интерфейс взаимодействия с [внешней средой](/specification/requirements/query): методы обращения к серверу и web-api.
...
Адаптируют [внешнюю среду](/specification/requirements/query) AUT на совместимые
со `storyshots` [заглушки](/specification/requirements/query).
...
- `pure` — это условное обозначение компонента. Описывает [один из способов](/patterns/replace#подмена-через-инверсию)
  ...
  [внешних зависимостей](/specification/requirements/query).
