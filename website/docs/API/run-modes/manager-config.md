...
Конфигурация для менеджера `storyshots`. Используется при запуске в режимах [UI](/API/manager/runUI) и [CI](/API/manager/runInBackground).
...
Принимает [сервер preview](/modules/scheme#ipreviewserver).
