...
Externals - это устоявшийся термин для обозначения методов [внешней среды](/specification/requirements/query) и [хранилища](/specification/requirements/command).

## Тривиальные externals

<MetricsTip improves={[Metric.RegressionProtection, Metric.RefactoringAllowance]} />

Элементы компонентов [внешней среды](/specification/requirements/query) и [хранилища](/specification/requirements/command) подменяются в `storyshots`.

- _[Внешняя среда](/specification/requirements/query)_ - подменяется, чтобы исключить недетерминированность.
- _[Хранилище](/specification/requirements/command)_ - подменяется, чтобы исключить сайд-эффекты.
...
подмены [внешней среды](/specification/requirements/query) заглушками. Для этого, во втором примере подменяемый метод был сделан _скромным_,
теперь он не содержит особой логики и лишь выполняет простое делегирование серверу.

:::note
Вырезанная логика поднимается выше по стеку, попадая в [тестируемый слой](/modules/scheme#aut).
:::
...
[Сервер preview](#todo) должен собирать приложение идентичным целевой конфигурации образом.
...
Методы, подменяющие [внешнюю среду](/specification/requirements/query), в том числе и сами заглушки должны быть модульными. Это в частности означает что
...
`externals` в `storyshots` заключает в себе как методы мутирующие [хранилище](/specification/requirements/command), так и функции работающие
с [внешней средой](/specification/requirements/query). Очень часто, данные компоненты формируют пару из _команды_ (command) и _запроса_ (query):
...
:::note
Метод `removeUserById` относится к компоненту [хранилища](/specification/requirements/command), в то время как `getUsers` является частью [внешней среды](/specification/requirements/query).
:::
...
На проектах структура `externals` ([внешней среды](/specification/requirements/query) и [хранилища](/specification/requirements/command)) очень часто является вложенной:
