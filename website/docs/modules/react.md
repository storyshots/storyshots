...
Реализует [`клиент preview`](/modules/scheme#ipreviewdisplay) для `react` приложений:
...
Позволяет задавать поведение и логирование по умолчанию для [внешних зависимостей](/specification/requirements/query) приложения.

---

### createExternals

Основная фабрика, инициализирующая [внешние зависимости](/specification/requirements/query) приложения.
...
`@storyshots/react` дополнительно расширяет фабрику [it](/API/it) следующими методами:
...
Принимает `externals` и [StoryConfig](/API/story-elements/story-config):
...
Подготавливает [внешние зависимости](/specification/requirements/query) для истории.
...
Может также использоваться для разметки методов для логирования с помощью [Journal](/API/story-elements/journal):
