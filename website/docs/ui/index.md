...
1. Запускает функцию `render` истории и отображает её на экране.
2. Выполняет действия, описанные в `act`.
...
Чтобы включить эмуляцию, поставьте галочку в пункте "Apply to preview". Теперь рабочая область будет автоматически
