...
Относится к секции [*аргументов*](/specification/requirements/borders#define-boundaries).
...
## Изменяемое окружение {#mutable-environment}
...
* Избавиться от внешнего состояния, поместив интерфейс доступа в слой [аргументов](/specification/requirements/borders#define-boundaries).
...
## Запросы к серверу {#server-requests}
...
## События среды {#environment-events}
...
как есть, то она увеличит время выполнения тестов, что сильно повредит [быстродействию](/specification/metrics#performance). 
...
## Связь с библиотекой {#library-integration}
...
* Для JS анимаций следует использовать [previewing](/modules/react#previewing) флаг.
* Компоненты Web API, такие, как `setTimeout`, `Date` и другие, можно заменить [инвазивным](/patterns/replace#mocking-through-inversion) и [неинвазивным](/patterns/replace#mocking-through-side-effects)
  способом.
