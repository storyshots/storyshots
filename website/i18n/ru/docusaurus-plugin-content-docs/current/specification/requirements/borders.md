...
# Границы тестирования {#test-boundaries}
...
## Определение границ {#define-boundaries}
...
```mermaid
flowchart LR
  E["Эталон"] -- "Содержит" --> S["Слепок"]
  AUT["AUT"] -- "Генерирует" --> S2["Слепок'"]
  T["Тест"] -- "Сравнивает" --> C["Слепок / Слепок'"]
