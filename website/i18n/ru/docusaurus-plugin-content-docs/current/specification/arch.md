...
## Менеджер историй {#stories-manager}
...
## Адаптеры внешней среды {#external-environment-adapters}
...
- `native` — это условное обозначение компонента. Описывает [один из способов](/patterns/replace#mocking-through-inversion)
  самостоятельной подмены зависимостей.
- [`@storyshots/msw-externals`](/modules/msw) -
  реализует [не инвазивный метод](/patterns/replace#mocking-through-side-effects)
