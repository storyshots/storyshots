...
## Игнорирование query {#ignoring-query}

<MetricsTip improves={[Metric.RefactoringAllowance, Metric.Maintainability]} degrades={[Metric.RegressionProtection]} />

`storyshots` предоставляет методы для отслеживания вызовов функций, а именно [журнал](/specification/requirements/command#verification-method). Записывать можно абсолютно
...
:::tip
[Cайд-эффект](/specification/requirements/command#side-effects) - это не просто результат работы выходящий за пределы функции, в рамках спецификации это ещё и
...
## Нестабильные представления {#unstable-views}
...
выполняет [подмену через сайд-эффекты](/patterns/replace#mocking-through-side-effects).
:::

:::warning Внимание
Существует также альтернатива ввиде подмены API через [инверсию зависимостей](/patterns/replace#mocking-through-inversion), однако данный метод не является
